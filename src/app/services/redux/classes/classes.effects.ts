    ...
    public loadForBlock$: Observable<Action> = this._actions$
        ...
        .switchMap(blockId => this._repository.getForBlock(blockId)
            ...
            .catch(response => response.status === 404
                   ? Observable.of(new classActions.LoadClassesForBlockSucceded([])) as Observable<Action>
                   : Observable.of(new classActions.LoadClassesForBlockFailed(response)) as Observable<Action>
            )
        );
