...
export class BlocksEffects {
    ...
    public loadSelected$: Observable<Action> = this._actions$
        ...
        .map((action: blockActions.LoadSelectedBlock) => action.id)
    ...
    public update$: Observable<Action> = this._actions$
        ...
        .map((action: blockActions.UpdateBlock) => blockToDto(action.block))
    ...
    public create$: Observable<Action> = this._actions$
        .ofType(blockActions.CREATE)
        .map((action: blockActions.CreateBlock) => blockToDto(action.block))
        .switchMap(block => this._repository.create(block)
            .map(updatedBlock => new blockActions.CreateBlockSucceded(block.id))
            .catch(response => response.status === 400
                ? Observable.of(new blockActions.CreateBlockValidationError(dtoToValidation(response.json()))) as Observable<Action>
                : Observable.of(new blockActions.CreateBlockFailed(response)) as Observable<Action>)
        );

    @Effect()
    public createSucceded$: Observable<Action> = this._actions$
        .ofType(blockActions.CREATE_SUCCEDED)
        .map((action: blockActions.UpdateBlockSucceded) => new blockActions.LoadSelectedBlock(action.id));
